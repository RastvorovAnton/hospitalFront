...
const FilterComponent = () => {
	...
	const { complaints, setComplaints, setOpenFilter } = useContext(MyContext);
  ...
  const oriiginalOrderReseption = () => {
    axios
      .get("http://localhost:8000/allApointments", {
        headers: {
  ...
  const filterApointments = () => {
    ...
    if (!start && !end) return setComplaints([...complaints]);
    ...
    return setComplaints([...complaints]);
  };
